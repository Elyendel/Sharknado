...
const utilsDrawing = (function() {
  ...
  return {
    ...
    addTornadoContour: function(raycaster, camera, xPosition ,yPosition, drawingData, scene){
      raycaster.setFromCamera(new THREE.Vector2(xPosition,yPosition),camera);
      const nbPoints = drawingData.drawing3DPoints.length;
      // ajout de hauteur a chaque point
      const t = nbPoints;
      const intersects = raycaster.intersectObjects( drawingData.drawingObjects );

      const intersection = intersects[0];
      const point = intersection.point.clone();
      point.z += Math.pow(t, 6/8);
      drawingData.drawing3DPoints.push(point);

      utilsData.addLine(drawingData, scene);
    },

    addTornado: function(drawingData, scene){
      // methode : la curve est la courbe du tube
      const curve = new THREE.CatmullRomCurve3(drawingData.drawing3DPoints);

      const tubeGeometry = new THREE.TubeGeometry(curve, 64, 2);
      const tubeMaterial = new THREE.MeshPhongMaterial( { color: 0x494949 } );
      const tube = new THREE.Mesh(tubeGeometry, tubeMaterial);
      tube.name = "tornado";
      scene.add(tube);
      drawingData.drawingObjects.push(tube);
    ...
    removeMesh: function(raycaster, camera, xPosition ,yPosition, drawingData, scene){
      ...
      if( nbrIntersection>0 ) {
        ...
        if(intersection.object.name != "plane"){
          ...
          if(intersection.object.name == "body" || intersection.object.name == "tornado"){
